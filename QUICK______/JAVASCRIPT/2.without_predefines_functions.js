...
function countVowels(s) {
  let c = 0;
  let v = { 'a': 1, 'e': 1, 'i': 1, 'o': 1, 'u': 1, 'A': 1, 'E': 1, 'I': 1, 'O': 1, 'U': 1 };
...
function mergeArrays(a1, a2) {
  ...
  while (i < a1.length) r[r.length] = a1[i++];
  while (j < a2.length) r[r.length] = a2[j++];
  return r;
}
...
function capitalize(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    if (s[i] === ' ') {
      r += ' ';
      nw = true;
    } else if (nw && s[i] >= 'a' && s[i] <= 'z') {
      r += String.fromCharCode(s.charCodeAt(i) - 32);
      nw = false;
    } else {
      r += s[i];
      nw = false;
...
function deepEqual(o1, o2) {
  ...
  let k1 = 0, k2 = 0;
  for (let k in o1) {
    if (Object.prototype.hasOwnProperty.call(o1, k)) k1++;
  }
  for (let k in o2) {
    if (Object.prototype.hasOwnProperty.call(o2, k)) k2++;
  }
...
function camelToSnake(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    if (s[i] >= 'A' && s[i] <= 'Z') {
      r += "_" + String.fromCharCode(s.charCodeAt(i) + 32);
    } else {
      r += s[i];
    }
  }
...
function isRotation(s1, s2) {
  if (s1.length !== s2.length) return false;
  let t = s1 + s1;
  for (let i = 0; i <= t.length - s2.length; i++) {
    let m = true;
    for (let j = 0; j < s2.length; j++) {
      if (t[i + j] !== s2[j]) m = false;
    }
    if (m) return true;
  }
  return false;
...
// 47. Bind
function customBind(f, c) {
  let ba = [];
  for (let i = 2; i < arguments.length; i++) {
    ba[ba.length] = arguments[i];
  }
  return function () {
    let a = [];
...
function curry(f) {
  return function c() {
    ...
    return function () {
      ...
      for (let i = 0; i < arguments.length; i++) {
        na[na.length] = arguments[i];
      }
      return c.apply(this, na);
    };
  };
}
...
function extNums(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    if (s[i] >= '0' && s[i] <= '9') {
      ...
      r[r.length] = Number(cn);
      cn = "";
    }
  }
  if (cn !== "") r[r.length] = Number(cn);
  return r;
}
